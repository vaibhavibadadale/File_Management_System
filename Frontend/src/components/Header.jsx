...
const Header = ({ user, currentTheme, onThemeToggle, onLogout }) => {
    ...
    const handleNotificationClick = async (notification) => { // Fixed the 'cconst' typo here
    try {
        ...
        if (type.includes('REQUEST') || type.includes('TRANSFER') || type.includes('DELETE')) {
            navigate('/pending'); // This matches the route in your App.js
                ...
                <div className="position-relative" ref={notifyRef}>
                    ...
                    {isNotifyOpen && (
                        <div className={`position-absolute end-0 mt-2 shadow-lg rounded-3 overflow-hidden ${currentTheme === 'dark' ? 'bg-dark border border-secondary' : 'bg-white border'}`} 
                             style={{ width: "400px", zIndex: 1060 }}> {/* Width increased to 400px */}
                            ...
                            <div style={{ maxHeight: '450px', overflowY: 'auto' }}>
                                {notifications.length === 0 ? (
                                    ...
                                    /* .slice(0, 5) ensures only the 5 latest appear in pop-up */
                                    notifications.slice(0, 5).map(n => (
                                        <div 
                                            key={n._id} 
                                            className={`px-3 py-3 border-bottom small transition-all ${currentTheme === 'dark' ? 'bg-dark text-light border-secondary' : 'bg-white text-dark'}`}
                                            onClick={() => handleNotificationClick(n)}
                                            style={{ cursor: 'pointer', transition: '0.2s background' }}
                                            onMouseEnter={(e) => e.currentTarget.style.backgroundColor = currentTheme === 'dark' ? '#2d3748' : '#f8f9fa'}
                                        ...
                                        >
                                            ...
                                            <div className={`${currentTheme === 'dark' ? 'text-light-50' : 'text-muted'}`} style={{ fontSize: '0.85rem', lineHeight: '1.4' }}>
                                                {n.message}
                                            </div>
                                            <div className="mt-2 text-end" style={{ fontSize: '0.65rem', opacity: 0.6 }}>
                ...
                <div className="position-relative" ref={dropdownRef}>
                    ...
                    {isDropdownOpen && (
                        <div className={`position-absolute end-0 mt-2 shadow-lg rounded-3 py-2 ${currentTheme === 'dark' ? 'bg-dark border border-secondary text-light' : 'bg-white border text-dark'}`} style={{ width: "200px", zIndex: 1060 }}>
                            <Link to={`/user-files/${user?._id || user?.id}`} className="dropdown-item px-3 py-2 small text-reset text-decoration-none d-block">👤 My Profile</Link>
